...
| Deterministic tests | PASS. 43 deterministic tests passed; the separate optional live-service test was skipped, for 44 discovered tests total and no deterministic failures. Coverage includes centralized integer/one-decimal percentage formatting and rounding, menu-bar presentation, two-AI composition, persistence, icon-color tinting, exact interval limits, stale data, retry behavior, localization, day/hour/minute reset formatting, provider PNG resource loading, and accessory activation policy. |
...
| Claude Code | PASS (bridge). The provider parses the documented Claude Code status-line `rate_limits` payload (`five_hour` and `seven_day`) into remaining percentages and reset dates. It never reads credentials or calls private Anthropic endpoints. The local installation was not logged in during this run, so a live subscriber response remains an opt-in service check. |
...
| Manual refresh | PASS. The actual 다시 확인 button triggered a fresh Codex request and re-read the Claude status-line bridge when configured, updating the displayed percentage and last-checked time. |
