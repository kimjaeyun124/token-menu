...
- 메뉴바에 남은 사용량 표시: 기본적으로 5시간 한도와 정수 퍼센트를 사용합니다.
- 주간·선택적 월간 한도와 초기화 표시 설정: 남은 시간·현지 초기화 시각·둘 다·숨기기를 선택할 수 있습니다. 남은 시간은 0인 단위를 생략하고 하루 이상이면 일 단위까지 보여 줍니다.
- 다시 확인·설정·종료 버튼을 갖춘 간결한 사용량 창.
- 서비스별 표시 여부, 순서, 확인 주기 및 사용량 부족·Codex 작업 완료·토큰 한도 초기화 알림 설정.
...
- 로컬 Codex app-server에서 현재 작업 상태(작업 중·승인 대기·입력 대기)와 경과 시간 표시.
- Codex에서 직접 연 결과도 작업별 읽음 상태와 동기화합니다. 메뉴바 창을 열거나 ‘다시 확인’을 누르면 최신 읽음 상태가 반영되며, Codex 앱만 앞으로 가져와도 다른 미확인 작업이 사라지지는 않습니다.
- `사용 중인 AI 표시`를 끌 수 있으며, 표시를 꺼도 작업 완료 알림을 위한 백그라운드 감시는 유지됩니다.
- 설정에서 프로젝트별 위·아래 버튼으로 사용 중인 AI 작업 순서를 정할 수 있으며, 새 작업은 저장된 항목 뒤에 표시됩니다.

Token Menu는 서비스가 제공하는 사용량 한도를 표시합니다. 토큰 수를 추정하지 않으며, 정보가 없으면 0%가 아닌 확인할 수 없는 상태로 표시합니다.

로그인한 요금제의 Codex 응답에 월간 한도가 포함되면 Token Menu가 함께 표시합니다. 현재 Codex 응답에 월간 창이 없을 때는 임의로 월간 값을 만들지 않습니다.

Codex app-server 데몬을 사용할 수 있으면 사용량 창에 모든 활성 작업, 대기 상태와 경과 시간을 표시합니다. 읽기 전용 연결이므로 Codex 작업에 프롬프트를 보내거나 작업을 변경하지 않습니다.
Token Menu는 공식 ChatGPT/Codex 앱에 포함된 Codex 실행 파일을 우선 사용합니다. quarantine 또는 AppTranslocation 상태인 실행 파일은 macOS XProtect가 실행하도록 요청하지 않고 건너뜁니다. 감지된 실행 파일이 모두 차단된 경우 공식 앱에서 Codex를 설치하거나 업데이트한 뒤 다시 확인해 주세요.

데스크톱 Codex가 별도 stdio 서버를 사용할 때는 로컬 작업 생명주기 기록도 확인해 진행 중인 작업을 감지합니다. 여러 작업은 작업 폴더명과 함께 각각 표시하며, 프롬프트와 도구 내용은 읽지 않습니다.

읽음 상태는 Codex 파일을 변경하지 않고 로컬 데스크톱 기록에서 확인합니다. 데스크톱에 등록된 작업에만 적용하며, 완료 직후에는 기록 저장 시간을 짧게 기다립니다. 기록이 없거나 지원되지 않는 형식이거나 계정·호스트를 구분할 수 없으면 완료 항목을 유지합니다. 이때도 작업 행을 눌러 해당 결과를 열 수 있습니다.
...
다운로드한 DMG나 다운로드 폴더에서 직접 실행하지 말고, 반드시 **응용 프로그램** 폴더로 복사한 앱을 실행하세요. quarantine이 붙은 앱을 직접 열면 macOS가 AppTranslocation 임시 경로에서 실행해 Codex app-server에 연결하지 못할 수 있습니다. 현재 임시 서명 빌드는 quarantine을 스스로 제거할 수 없으므로, 계속 임시 보안 경로로 실행되면 **시스템 설정 → 개인정보 보호 및 보안 → 확인 없이 열기**를 선택한 뒤 응용 프로그램 폴더의 앱을 다시 실행하세요.
